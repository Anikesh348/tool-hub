...
public class MovieHubAutomationRoute {
  ...
  private MovieHubRequestPortalService requestPortalService;

  public MovieHubAutomationRoute(WebClient webClient, Dotenv dotenv, MongoDBClient mongoDBClient) {
  ...
  public void register(Router protectedRouter, Router adminRouter, Vertx vertx) {
    ...
    String movieHubPortalUrl =
        dotenv.get("MOVIEHUB_PORTAL_URL", "https://hostingfrompurava.xyz/moviehub");
    String movieHubAccessSecret =
        dotenv.get("MOVIEHUB_ACCESS_SECRET", dotenv.get("JWT_SECRET", ""));
    AiModel aiModel = AiModel.valueOf(dotenv.get("AI_MODEL"));
    AiClientFactory aiClientFactory =
        new AiClientFactory(List.of(new OpenAiClient(webClient, openAiUrl, openAiApiKey)));
    AiClient aiClient = aiClientFactory.getClient(aiModel);
    AddMediaControllerFactory addMediaControllerFactory =
        new AddMediaControllerFactory(
            List.of(
    ...
    MovieHubRequestPortalService movieHubRequestPortalService =
        new MovieHubRequestPortalService(
            mongoDBClient,
            ...
            sonarrBaseUrl,
            sonarrApiKey);
    this.requestPortalService = movieHubRequestPortalService;
    MovieHubAccessPortalService movieHubAccessPortalService =
        new MovieHubAccessPortalService(
            mongoDBClient,
            webClient,
            jellyfinBaseUrl,
            jellyfinPublicUrl,
    ...
    IntentStrategyFactory intentStrategyFactory =
        new IntentStrategyFactory(
            List.of(
  ...
  public MovieHubRequestPortalService requestPortalService() {
    return requestPortalService;
  }
}
