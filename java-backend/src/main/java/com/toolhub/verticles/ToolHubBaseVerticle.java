      ...
      vertx
          ...
          .onSuccess(
              mongoClient -> {
                ...
                StartupIndexes.ensure(mongoClient)
                    .onSuccess(ignored -> log.info("Mongo startup indexes are ready"))
                    .onFailure(error -> log.error("Unable to ensure Mongo startup indexes", error));
                this.client = WebClient.create(vertx);
                Router router = Router.router(vertx);
                router.get("/metrics").handler(RequestMetricsHandler::respond);
                router.route().handler(new RequestMetricsHandler());
                router
                    .route()
                    .handler(
                        CorsHandler.create()
                ...
                router.route().handler(BodyHandler.create().setBodyLimit(32L * 1024 * 1024));
                ...
                MovieHubAutomationRoute movieHubRoutes =
                    new MovieHubAutomationRoute(client, dotenv, mongoDBClient);
                movieHubRoutes.register(protectedRouter, adminRouter, vertx);

                UserManagement userManagement = new UserManagement(mongoDBClient, client, dotenv);
                SaveProduct saveProduct = new SaveProduct(mongoDBClient, client, vertx);
                YtDownloadProxyService ytDownloadProxyService =
                    new YtDownloadProxyService(client, vertx, mongoDBClient, dotenv);
                ...
                protectedRouter
                    .post("/leetcode/update-status")
                    .handler(new UpdateQuestionStatus(mongoDBClient)::handle);
                protectedRouter
                    .post("/leetcode/update-notes")
                    .handler(new UpdateQuestionNotes(mongoDBClient)::handle);
                protectedRouter
                    .post("/leetcode/delete")
                    .handler(new DeleteQuestion(mongoDBClient)::handle);

                protectedRouter.post("/yt/formats").handler(ytDownloadProxyService::handleFormats);

                // CRON ROUTES
                protectedRouter
                    .post("/yt/download/cronStart")
                    .handler(ytDownloadProxyService::handleCronStart);
                protectedRouter
                    .get("/yt/download/cronStart")
                    .handler(ytDownloadProxyService::handleCronStart);
                protectedRouter
                    .post("/yt/download/check")
                    .handler(ytDownloadProxyService::handleCheckAndUpdate);
                protectedRouter
                    .get("/yt/download/check")
                    .handler(ytDownloadProxyService::handleCheckAndUpdate);

                protectedRouter
                    .get("/yt/download/status/stream/:videoId")
                    .handler(ytDownloadProxyService::handleStatusStream);
                adminRouter.post("/yt/download/start").handler(ytDownloadProxyService::handleStart);
                adminRouter.post("/yt/download/add").handler(ytDownloadProxyService::handleAdd);
                adminRouter
                    .get("/yt/download/requests")
                    .handler(ytDownloadProxyService::handleListRequests);
                adminRouter
                    .delete("/yt/download/requests/:requestId")
                    .handler(ytDownloadProxyService::handleDeleteRequest);
                adminRouter
                    .get("/yt/download/status/:videoId")
                    .handler(ytDownloadProxyService::handleStatus);
                adminRouter
                    .get("/yt/download/status/stream/:videoId")
                    .handler(ytDownloadProxyService::handleStatusStream);
                adminRouter
                    .get("/yt/library/items")
                    .handler(ytDownloadProxyService::handleListLibraryItems);
                adminRouter
                    .delete("/yt/library/items/:itemId")
                    .handler(ytDownloadProxyService::handleDeleteLibraryItem);

                ParityRoutes.Services parityServices =
                    ParityRoutes.register(
                        router,
                        protectedRouter,
                        adminRouter,
                        vertx,
                        ...
                        mongoDBClient,
                        dotenv,
                        movieHubRoutes.requestPortalService());

                vertx
                    .deployVerticle(
                        new ToolHubSchedulerVerticle(
                            dotenv,
                            client,
                            parityServices.flights(),
                            movieHubRoutes.requestPortalService(),
                            parityServices.buzzWatch()))
                    .onSuccess(id -> log.info("ToolHubSchedulerVerticle deployed with ID: {}", id))
                    .onFailure(err -> log.error("Failed to deploy ToolHubSchedulerVerticle", err));
