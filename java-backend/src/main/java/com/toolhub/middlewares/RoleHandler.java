...
public class RoleHandler implements Handler<RoutingContext> {
  ...
  public void handle(RoutingContext context) {
    ...
    if (role == null) {
      Utility.buildResponse(context, 403, Utility.createErrorResponse("role missing in token"));
      return;
    }

    if (!allowedRoles.contains(role)) {
      Utility.buildResponse(context, 403, Utility.createErrorResponse("access denied"));
      return;
    }
