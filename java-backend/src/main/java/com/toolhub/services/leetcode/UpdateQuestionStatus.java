  ...
  public void handle(RoutingContext context) {
    ...
    if (questionId == null || newStatus == null) {
      Utility.buildResponse(
          context, 400, Utility.createErrorResponse("Missing questionId or status"));
      return;
    }

    JsonObject query = new JsonObject().put("questionId", questionId).put("userId", userId);
    JsonObject update =
        new JsonObject()
            .put(
                "$set",
    ...
    mongoDBClient
        ...
        .onFailure(
            fail -> {
              log.error(
                  "Failed to update status for question {}: {}", questionId, fail.getMessage());
              Utility.buildResponse(
                  context, 500, Utility.createErrorResponse("Error updating status"));
            });
  }
}
