          ...
          .onSuccess(
              records -> {
                if (records != null && !records.isEmpty()) {
                  ...
                  mongoDBClient
                      .updateRecord(query, update, "leetcode")
                      .onSuccess(
                          v ->
                              log.info(
                                  "Updated existing LeetCode question for title={} userId={}",
                                  title,
                                  userId))
                      .onFailure(
                          fail ->
                              log.error(
                                  "Failed to update question title={} userId={} : {}",
                                  title,
                                  userId,
                                  fail.getMessage()));
                } else {
                  ...
                  mongoDBClient
                      .insertRecord(doc, "leetcode")
                      .onSuccess(
                          i ->
                              log.info(
                                  "Inserted new LeetCode question for title={} userId={}",
                                  title,
                                  userId))
                      .onFailure(
                          insertFail ->
                              log.error(
                                  "Failed to insert question title={} userId={} : {}",
                                  title,
                                  userId,
                                  insertFail.getMessage()));
                }
              })
          .onFailure(
              fail ->
                  log.error(
                      "Failed to query existing LeetCode question for url={} userId={} : {}",
                      title,
                      userId,
                      fail.getMessage()));
