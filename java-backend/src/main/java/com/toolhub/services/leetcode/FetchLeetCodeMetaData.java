...
public class FetchLeetCodeMetaData {
  ...
  private Future<LeetCodeQuestion> makeFetchCall() {
    ...
    String query =
        "query getQuestionDetail($titleSlug: String!) { question(titleSlug: $titleSlug) { title difficulty stats acRate topicTags { name slug } } }";
    JsonObject payload =
        new JsonObject()
            .put("query", query)
            ...
            ar -> {
              if (ar.succeeded()) {
                ...
                if (status >= 200 && status < 300) {
                  ...
                  tagsPromise.fail(
                      "unexpected status: " + status + " body: " + response.bodyAsString());
                }
              } else {
