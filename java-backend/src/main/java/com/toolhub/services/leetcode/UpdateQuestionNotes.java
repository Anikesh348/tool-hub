  ...
  public void handle(RoutingContext context) {
    ...
    if (questionId == null || notes == null) {
      Utility.buildResponse(
          context, 400, Utility.createErrorResponse("Missing questionId or notes"));
      return;
    }
    ...
    mongoDBClient
        ...
        .onFailure(
            fail -> {
              log.error(
                  "Failed to update notes for question {}: {}", questionId, fail.getMessage());
              Utility.buildResponse(
                  context, 500, Utility.createErrorResponse("Error updating notes"));
            });
  }
}
