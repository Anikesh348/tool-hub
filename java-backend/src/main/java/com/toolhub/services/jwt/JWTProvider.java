...
public class JWTProvider {
  ...
  private static final long accessTokenTtlMinutes =
      parseLong(dotenv.get("ACCESS_TOKEN_TTL_MINUTES"), 15L);
  private static final long refreshTokenTtlDays =
      parseLong(dotenv.get("REFRESH_TOKEN_TTL_DAYS"), 3L);
  ...
  public static String generateRefreshToken(String userId, String role, String email) {
    return createToken(
        userId, role, email, TOKEN_TYPE_REFRESH, refreshTokenTtlDays * 24 * 60 * 60 * 1000);
  }
  ...
  private static String createToken(
      String userId, String role, String email, String tokenType, long ttlMillis) {
    Date now = new Date();
