...
public class LeetcodeBatchProcessor implements BatchProcessor<LeetCodeQuestion> {
  ...
  public static void process(
      MongoDBClient mongoDBClient, WebClient webClient, List<LeetCodeQuestion> questions) {
  ...
  public void handleBatch(int start, List<LeetCodeQuestion> questions) {
    ...
    List<LeetCodeQuestion> subListQuestions =
        questions.subList(start, Math.min(start + LIMIT, questions.size()));
    List<Future<LeetCodeQuestion>> productQueryFutures = new ArrayList<>();
    subListQuestions.forEach(
        question -> {
