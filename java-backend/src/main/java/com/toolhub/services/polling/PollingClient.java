...
public class PollingClient<T> {
  ...
  private void attempt(
      int attempt, Supplier<Future<T>> supplier, Predicate<T> predicate, Promise<T> promise) {
    ...
    try {
      supplier
              ...
              result -> {
                try {
                  if (predicate.test(result)) {
                    ...
                    log.error(
                        "Condition not met at attempt {}, scheduling next attempt", attempt + 1);
          ...
          .onFailure(
              err -> {
                log.warn("Polling attempt {} failed: {}", attempt + 1, err.getMessage());
                scheduleNext(attempt, supplier, predicate, promise);
              });
    } catch (Exception e) {
