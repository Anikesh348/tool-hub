...
public class AlertsValidator {
  ...
  public void checkForAlertsAndSend(JsonObject futureResult) {
    ...
    log.info(
        "{}, users will be alerted for priceDrop of: {}", toBeAlertedUsers, product.getProductId());
    userManagement
        .fetchUsersFromUserIds(new ArrayList<>(toBeAlertedUsers))
        .onFailure(
            fail -> {
