...
public class AddShowController implements AddMediaController {
  ...
  public AddShowController(
      WebClient webClient, String sonarrBaseUrl, String sonarrApiKey, Vertx vertx) {
  ...
  private void downloadShow(
    ...
    ShowDownloadClient.get(webClient, sonarrApiKey, sonarrBaseUrl)
        ...
        .onSuccess(
            res -> {
              ...
              log.info(
                  "Successfully queued downloads for seriesId={} title={}",
                  seriesId,
                  lookUpDTO.getTitle());
            })
        .onFailure(
            fail -> {
              ...
              log.error(
                  "Failed to queue downloads for seriesId={} title={}: {}",
                  seriesId,
                  lookUpDTO.getTitle(),
                  msg,
                  fail);
  ...
  public Future<Void> addContent(LookUpDTO lookUpDTO) {
    ...
    log.debug(
        "addContent called for title={} seasons={}", lookUpDTO.getTitle(), lookUpDTO.getSeason());
    lookUpClient
        ...
        .onSuccess(
            lookupResults -> {
              ...
              try {
                ...
                GetShowClient.get(webClient, sonarrBaseUrl, sonarrApiKey)
                    .getSeries()
                    .onSuccess(
                        seriesRes -> {
                          try {
                            JsonObject neededSeries =
                                seriesRes.stream()
                            ...
                            if (neededSeries == null) {
                              log.info(
                                  "Series not found in Sonarr for tvdbId={} title={}. Adding series.",
                                  tvdbId,
                                  lookUpDTO.getTitle());
                              // add series
                              AddMediaClient.add(
                                      webClient,
                                      sonarrBaseUrl + addShow,
                                      sonarrApiKey,
                                      addShowPayload)
                                  .onSuccess(
                                      addMediaResp -> {
                                        try {
                                          ...
                                          log.info(
                                              "Added series to Sonarr: title={} id={}",
                                              lookUpDTO.getTitle(),
                                              seriesId.get());
                                          PollingClient<JsonObject> pollingClient =
                                              new PollingClient<>(vertx, 1500, 15);
                                          Predicate<JsonObject> predicate =
                                              (series) ->
                                                  series
                                                          .getJsonObject(
                                                              "statistics", new JsonObject())
                                                          .getInteger("episodeCount", 0)
                                                      > 0;
                                          pollingClient
                                              .poll(
                                                  () ->
                                                      GetSelectSeriesClient.get(
                                                              webClient,
                                                              sonarrBaseUrl,
                                                              sonarrApiKey)
                                                          .getSeriesDetails(seriesId.get()),
                                                  predicate)
                                              .onSuccess(
                                                  res -> {
                                                    log.info(
                                                        "series has been added, will trigger download now...");
                                                    downloadShow(
                                                        lookUpDTO,
                                                        null,
                                                        seriesId.get(),
                                              ...
                                              .onFailure(
                                                  fail -> {
                                                    log.error(
                                                        "failure in queueing the series for download");
                                                    addContentPromise.fail(
                                                        "Unable to add this show");
                                                  });

                                        } catch (Exception e) {
                                          String msg = safeMessage(e);
                                          log.error(
                                              "Error handling addMediaResp for title={}: {}",
                                              lookUpDTO.getTitle(),
                                              msg,
                                              e);
                                          addContentPromise.fail(msg);
                                  ...
                                  .onFailure(
                                      addSeriesFail -> {
                                        ...
                                        log.error(
                                            "Failed to add series for title={} : {}",
                                            lookUpDTO.getTitle(),
                                            msg,
                                            addSeriesFail);
                                        addContentPromise.fail(msg);
                              ...
                              GetSeriesResponse getSeriesResponse =
                                  Utility.castToClass(neededSeries, GetSeriesResponse.class);
                              seriesId.set(getSeriesResponse.getId());
                              log.info(
                                  "Series already exists in Sonarr: title={} id={}",
                                  lookUpDTO.getTitle(),
                                  seriesId.get());
                              downloadShow(
                                  lookUpDTO,
                            ...
                            String msg = safeMessage(e);
                            log.error(
                                "Error processing series list for title={}: {}",
                                lookUpDTO.getTitle(),
                                msg,
                                e);
                            addContentPromise.fail(msg);
                    ...
                    .onFailure(
                        seriesFail -> {
                          ...
                          log.error(
                              "Failed to fetch series list from Sonarr for title={}: {}",
                              lookUpDTO.getTitle(),
                              msg,
                              seriesFail);
                          addContentPromise.fail(msg);
                ...
                String msg = safeMessage(e);
                log.error(
                    "Unexpected error while preparing add payload for title={}: {}",
                    lookUpDTO.getTitle(),
                    msg,
                    e);
                addContentPromise.fail(msg);
  ...
  private JsonObject selectShowLookupResult(JsonArray lookupResults, LookUpDTO lookUpDTO) {
    ...
    if (requestedTvdbId != null) {
      ...
      log.warn(
          "No show lookup match found for tvdbId={} title={}",
          requestedTvdbId,
          lookUpDTO.getTitle());
      return null;
    }
    ...
    if (requestedImdbId != null && !requestedImdbId.isBlank()) {
      ...
      log.warn(
          "No show lookup match found for imdbId={} title={}",
          requestedImdbId,
          lookUpDTO.getTitle());
      return null;
    }
