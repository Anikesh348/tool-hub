...
public class MediaExistsIntentStrategy implements IntentStrategy {
  ...
  public MediaExistsIntentStrategy(
  ...
  public Future<IntentResponse> automate(ConversationContext context, String userInput) {
    ...
    JsonObject parseRequest =
        OpenAiRequestBuilder.buildPayload(
            context.getMediaState(), userInput, Intent.CHECK_MEDIA_EXISTS);
    aiClient
        ...
        .onSuccess(
            llmResponse -> {
              ...
              if (title == null || title.isBlank()) {
                response.setMessage(
                    "Please share the exact movie or series title you want me to check.");
  ...
  private String buildSingleTypeMessage(String title, MediaType mediaType, JsonArray results) {
    ...
    builder
        .append("Yes, \"")
        .append(title)
        .append("\" exists on the server as a ")
        .append(mediaLabel)
        .append(".\n");
    ...
    if (hasMovies) {
      ...
      for (int i = 0; i < movieResults.size(); i++) {
        builder
            .append(i + 1)
            .append(". ")
            .append(formatTitle(movieResults.getJsonObject(i)))
            .append("\n");
      }
    }
    if (hasShows) {
