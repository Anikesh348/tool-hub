...
public class RegexIntentResolver {
  ...
  private static final Pattern CHECK_MEDIA_EXISTS =
      Pattern.compile(
          "(?i)(\\bdoes\\b.+\\bexist\\b|\\b(is|check|find|search|lookup|look\\s*up)\\b.+\\b(movie|movies|show|series|tv)\\b)");

  private static final Pattern DELETE_MEDIA =
      Pattern.compile(
          "(?i)\\b(delete|remove|uninstall|erase)\\b.+\\b(movie|movies|show|series|tv|media|library)\\b");
