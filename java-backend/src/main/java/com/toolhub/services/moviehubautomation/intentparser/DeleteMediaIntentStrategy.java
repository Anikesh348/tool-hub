...
public class DeleteMediaIntentStrategy implements IntentStrategy {
  ...
  public DeleteMediaIntentStrategy(
      AiClient aiClient, MovieHubRequestPortalService movieHubRequestPortalService) {
  ...
  public Future<IntentResponse> automate(ConversationContext context, String userInput) {
    ...
    JsonObject parseRequest =
        OpenAiRequestBuilder.buildPayload(context.getMediaState(), userInput, Intent.DELETE_MEDIA);
    aiClient
        .makeAiCall(parseRequest)
        .onSuccess(
            llmResponse -> {
              LLMResponse.Query query = llmResponse.getQuery();
              String title = normalizeTitle(query == null ? null : query.getTitle());
              MediaType mediaType =
                  query != null && query.getMediaType() != null
                      ? query.getMediaType()
                      : MediaType.UNKNOWN;
              ...
              if (title == null || title.isBlank()) {
                response.setMessage(
                    "Please share the exact movie or series title you want to delete.");
              ...
              if (MediaType.MOVIES.equals(mediaType) || MediaType.SHOWS.equals(mediaType)) {
                movieHubRequestPortalService
                    ...
                    .onSuccess(
                        availableItems ->
                            respondWithMatches(context, title, mediaType, availableItems, promise))
                    .onFailure(
                        fail -> {
              ...
              Future<JsonArray> movieFuture =
                  movieHubRequestPortalService
                      .getAvailableMedia(MediaType.MOVIES)
                      .recover(fail -> Future.succeededFuture(new JsonArray()));
              Future<JsonArray> showFuture =
                  movieHubRequestPortalService
                      .getAvailableMedia(MediaType.SHOWS)
                      .recover(fail -> Future.succeededFuture(new JsonArray()));

              CompositeFuture.all(movieFuture, showFuture)
                  .onSuccess(
                      result -> {
                        ...
                        if (options.isEmpty()) {
                          combinedResponse.setMessage(
                              "No matching movie or series was found for \"" + title + "\".");
                          context.setCompleted(true);
  ...
  private Future<IntentResponse> handleSelection(
      ConversationContext context, String userInput, Promise<IntentResponse> promise) {
    IntentResponse response = new IntentResponse();
    JsonArray options = context.getSelectionOptions();
    if (options == null || options.isEmpty()) {
      context.setAwaitingSelection(false);
      response.setMessage(
          "I do not have any delete candidates right now. Please share the title again.");
      promise.complete(response);
      return promise.future();
    }
    ...
    movieHubRequestPortalService
        .deleteAvailableMediaItem(selectedMediaType, mediaId, true, false)
        .onSuccess(
            deleteResult -> {
        ...
        .onFailure(
            fail -> {
              response.setMessage(
                  "Failed to delete \"" + selectedTitle + "\": " + fail.getMessage());
              context.setCompleted(true);
              promise.complete(response);
            });
    return promise.future();
  ...
  private void respondWithMatches(
    ...
    if (matches.isEmpty()) {
      response.setMessage(
          "No matching " + mediaTypeLabel(mediaType) + " was found for \"" + title + "\".");
      context.setCompleted(true);
      promise.complete(response);
      return;
    }
  ...
  private JsonArray buildOptions(JsonArray results, MediaType mediaType) {
    ...
    for (int i = 0; i < results.size(); i++) {
      ...
      Integer mediaId =
          mediaType == MediaType.MOVIES ? item.getInteger("radarrId") : item.getInteger("sonarrId");
  ...
  private JsonArray filterAvailableByTitle(JsonArray availableItems, String queryTitle) {
    ...
    for (Object item : availableItems) {
      ...
      if (normalizedItemTitle.contains(normalizedQuery)
          || normalizedQuery.contains(normalizedItemTitle)) {
