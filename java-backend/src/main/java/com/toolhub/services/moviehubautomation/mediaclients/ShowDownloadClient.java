...
public class ShowDownloadClient {
  ...
  private Future<Void> downloadSeason(Integer seriesId, Integer seasonNumber, String title) {
    ...
    try {
      ...
      webClient
          ...
          .onSuccess(
              res -> {
                ...
                if ((statusCode >= 200 && statusCode < 300) || statusCode == 409) {
                  log.info(
                      "Queued season {} for seriesId={} title={} status={}",
                      seasonNumber,
                      seriesId,
                      title,
                      statusCode);
                ...
                log.warn(
                    "Season search command failed status={} seriesId={} season={} title={} body={}",
                    statusCode,
                    seriesId,
                    seasonNumber,
                    title,
          ...
          .onFailure(
              fail -> {
                ...
                log.error(
                    "Error while queuing season {} for seriesId={} title={}: {}",
                    seasonNumber,
                    seriesId,
                    title,
                    msg,
      ...
      log.error(
          "Unexpected error building/starting request for season {} seriesId={} title={}: {}",
          seasonNumber,
          seriesId,
          title,
          msg,
  ...
  private Future<Void> downloadSeries(Integer seriesId, String title) {
    Promise<Void> promise = Promise.promise();
    try {
      JsonObject request = new JsonObject().put("name", "SeriesSearch").put("seriesId", seriesId);
      log.info("calling: {} with request: {}", downloadSeriesUrl, request);
      webClient
          .postAbs(downloadSeriesUrl)
          .putHeader("x-api-key", apiKey)
          ...
          .onSuccess(
              res -> {
                ...
                if ((statusCode >= 200 && statusCode < 300) || statusCode == 409) {
                  log.info(
                      "Queued series search for seriesId={} title={} status={}",
                      seriesId,
                      title,
                      statusCode);
                  promise.complete();
                ...
                String body = res.bodyAsString();
                log.warn(
                    "Series search command failed status={} seriesId={} title={} body={}",
                    statusCode,
                    seriesId,
                    title,
                    body);
          ...
          .onFailure(
              fail -> {
                String msg = safeMessage(fail);
                log.error(
                    "Error while queuing series search for seriesId={} title={}: {}",
                    seriesId,
                    title,
                    msg,
                    fail);
      ...
      String msg = safeMessage(e);
      log.error(
          "Unexpected error building/starting series search for seriesId={} title={}: {}",
          seriesId,
          title,
          msg,
          e);
  ...
  public Future<Void> downloadSeasons(
      LookUpDTO lookUpDTO, GetSeriesResponse getSeriesResponse, Integer seriesId) {
    ...
    List<Integer> userSeasonRequest =
        (lookUpDTO == null || lookUpDTO.getSeason() == null) ? List.of() : lookUpDTO.getSeason();
    ...
    if (getSeriesResponse != null && getSeriesResponse.getSeason() != null) {
      try {
        ...
        for (GetSeriesResponse.Season season : getSeriesResponse.getSeason()) {
          ...
          Integer fileCount =
              season.getStatistics() == null ? null : season.getStatistics().getEpisodeFileCount();
          Integer episodeCount =
              season.getStatistics() == null ? null : season.getStatistics().getEpisodeCount();
          ...
          if (needsDownload) {
            ...
            log.debug(
                "Skipping season {} for seriesId={} title={} because files={} episodes={}",
                seasonNum,
                seriesId,
                title,
                fileCount,
        ...
        String msg = safeMessage(e);
        log.error(
            "Error while filtering seasons for download for seriesId={} title={}: {}",
            seriesId,
            title,
            msg,
            e);
    ...
    if (seasonList.isEmpty()) {
      if (userSeasonRequest.isEmpty()) {
        log.info(
            "No specific seasons requested for seriesId={} title={}; queueing full series search",
            seriesId,
            title);
    ...
    for (Integer season : seasonList) {
      try {
        ...
        log.debug(
            "Requested queueing for season {} on seriesId={} title={}", season, seriesId, title);
      } catch (Exception e) {
        String msg = safeMessage(e);
        log.error(
            "Failed to request download for season {} seriesId={} title={}: {}",
            season,
            seriesId,
            title,
            msg,
