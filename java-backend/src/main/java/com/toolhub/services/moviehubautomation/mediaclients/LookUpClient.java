      ...
      client
          ...
          .onSuccess(
              res -> {
                ...
                log.debug(
                    "Lookup response received with statusCode={} for query={}",
                    statusCode,
                    lookupQuery);
                if (statusCode >= 200 && statusCode < 300) {
                  try {
                    ...
                    log.info(
                        "Lookup successful for query={}, found {} result(s)",
                        lookupQuery,
                        arrResponse.size());
                    lookupPromise.complete(arrResponse);
                  } catch (Exception processingException) {
                    String errorMsg =
                        processingException.getCause() != null
                            ? processingException.getCause().getMessage()
                            : processingException.getMessage();
                    log.error(
                        "Exception while processing lookup response for query={}: {}",
                        lookupQuery,
                        errorMsg);
                  ...
                  log.warn(
                      "Lookup returned non-success status {} for query={}: {}",
                      statusCode,
                      lookupQuery,
                      res.bodyAsString());
