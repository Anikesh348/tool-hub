...
public class MovieHubAccessPortalService {
  ...
  public MovieHubAccessPortalService(
    ...
    this.jellyfinPublicUrl =
        trimTrailingSlash(
            jellyfinPublicUrl == null || jellyfinPublicUrl.isBlank()
                ? jellyfinBaseUrl
                : jellyfinPublicUrl);
    this.jellyfinApiKey = jellyfinApiKey;
    this.movieHubPortalUrl =
        trimTrailingSlash(
  ...
  public void handleAccessGuard(RoutingContext context) {
    ...
    if (path.startsWith("/v2/moviehub/access") || "/v2/moviehub/reconcile-downloads".equals(path)) {
      context.next();
      return;
    }
            ...
            hasAccess -> {
              ...
              Utility.buildResponse(
                  ...
                  Utility.createErrorResponse(
                      "moviehub access is not approved. Please request access first."));
            })
        .onFailure(
            fail -> {
  ...
  public void handleGetAccessUserMapping(RoutingContext context) {
    ...
    fetchUserById(userId)
        .compose(
            user -> {
              ...
              return CompositeFuture.all(
                      fetchAccessUserByEmail(email), fetchLatestRequestForEmail(email))
                  .map(
                      result -> {
                        ...
                        boolean showTemporaryPasswordNotice =
                            mapping.getValue("passwordResetConfirmedAt") == null;
                        return new JsonObject()
                            .put("userId", userId)
                            .put("exists", true)
        ...
        .onSuccess(
            response -> {
              log.info(
                  "Fetched moviehub access-user mapping for userId={} exists={} status={}",
                  userId,
        ...
        .onFailure(
            fail -> {
              String errorMessage =
                  resolveErrorMessage(fail, "failed to fetch moviehub user mapping");
  ...
  public void handlePlay(RoutingContext context) {
    JsonObject body = context.body().asJsonObject();
    String username = body == null ? "" : body.getString("username", "").trim();
    if (username.isBlank()) {
      Utility.buildResponse(
          context, 400, Utility.createErrorResponse("MovieHub username is required"));
      return;
    }
    Future<Boolean> authorized =
        Role.ADMIN.name().equalsIgnoreCase(context.get("role"))
            ? Future.succeededFuture(true)
            : mongoDBClient
                .queryRecords(
                    new JsonObject()
                        .put("userId", context.get("userId"))
                        .put("movieHubUserNameLower", username.toLowerCase(Locale.ROOT))
                        .put("active", true),
            ...
            allowed -> {
              if (!allowed) {
                Utility.buildResponse(
                    context,
                    403,
                    Utility.createErrorResponse("MovieHub access mapping was not found"));
                return;
              }
              fetchJellyfinUserByName(username)
                  .compose(
                      user -> {
                        if (user == null || user.getString("Id", "").isBlank())
                          return Future.failedFuture("MovieHub user is not available in Jellyfin");
                        return resolvePlayableItem(body)
                            .compose(
                                item ->
                                    findControllableSession(user.getString("Id"))
                                        .compose(
                                            sessionId ->
                                                webClient
                                                    .postAbs(
                                                        jellyfinBaseUrl
                                                            ...
                                                            + sessionId
                                                            + "/Playing")
                                                    .addQueryParam("playCommand", "PlayNow")
                                                    .addQueryParam("itemIds", item.getString("Id"))
                                                    .addQueryParam("startPositionTicks", "0")
                                                    .putHeader(
                                                        "authorization",
                                                    ...
                                                    .compose(
                                                        response ->
                                                            response.statusCode() >= 200
                                                                ...
                                                                : Future.failedFuture(
                                                                    "Jellyfin could not start playback: "
                  ...
                  .onSuccess(
                      item ->
                          Utility.buildResponse(
                              ...
                              Utility.createSuccessResponse(
                                  new JsonObject()
                                      .put("message", "Playback started")
                                      .put("itemId", item.getString("Id"))
                                      .put(
                                          "title",
                                          item.getString("Name", body.getString("title"))))))
                  .onFailure(
                      error -> {
                        ...
                        int status =
                            message != null
                                    && (message.contains("not available")
                                        ...
                                        || message.contains("Open the Watch tab")
                                        || message.contains("No playable episodes"))
                                ? 404
                                : 500;
                        Utility.buildResponse(
                            context, status, Utility.createErrorResponse(message));
                      });
            })
        .onFailure(
            error ->
                Utility.buildResponse(
                    context, 500, Utility.createErrorResponse(error.getMessage())));
  }

  private Future<JsonObject> resolvePlayableItem(JsonObject body) {
    String title = body.getString("title", "").trim();
    String mediaType = body.getString("mediaType", "MOVIES").trim().toUpperCase(Locale.ROOT);
    if (title.isBlank()) return Future.failedFuture("Title is required");
    String includeType = "SHOWS".equals(mediaType) ? "Series" : "Movie";
    return webClient
        .getAbs(jellyfinBaseUrl + "/Items")
        .addQueryParam("Recursive", "true")
        .addQueryParam("SearchTerm", title)
        .addQueryParam("IncludeItemTypes", includeType)
        .addQueryParam("Fields", "ProviderIds,ProductionYear")
        ...
        .compose(
            response -> {
              if (response.statusCode() < 200 || response.statusCode() >= 300)
                return Future.failedFuture("failed to search Jellyfin library");
              JsonArray candidates =
                  response.bodyAsJsonObject().getJsonArray("Items", new JsonArray());
              JsonObject match =
                  candidates.stream()
                      .filter(JsonObject.class::isInstance)
                      .map(JsonObject.class::cast)
                      .max(Comparator.comparingInt(item -> itemMatchScore(item, body, title)))
                      .orElse(null);
              if (match == null || itemMatchScore(match, body, title) <= 0)
                return Future.failedFuture(title + " was not found in the Jellyfin library");
              if (!"SHOWS".equals(mediaType)) return Future.succeededFuture(match);
              return webClient
                  .getAbs(jellyfinBaseUrl + "/Items")
                  .addQueryParam("ParentId", match.getString("Id"))
                  .addQueryParam("Recursive", "true")
                  .addQueryParam("IncludeItemTypes", "Episode")
                  .addQueryParam("IsMissing", "false")
                  .addQueryParam("SortBy", "ParentIndexNumber,IndexNumber")
                  .addQueryParam("SortOrder", "Ascending")
                  ...
                  .compose(
                      episodes -> {
                        JsonArray items =
                            episodes.bodyAsJsonObject().getJsonArray("Items", new JsonArray());
                        return items.isEmpty()
                            ? Future.failedFuture("No playable episodes were found for " + title)
                            : Future.succeededFuture(items.getJsonObject(0));
                      });
            });
  }

  private int itemMatchScore(JsonObject item, JsonObject body, String title) {
    int score = item.getString("Name", "").trim().equalsIgnoreCase(title) ? 5 : 0;
    JsonObject providers = item.getJsonObject("ProviderIds", new JsonObject());
    for (String key : List.of("Imdb", "Tmdb", "Tvdb")) {
      String expected = body.getString(key.toLowerCase(Locale.ROOT) + "Id", "").trim();
      if (!expected.isBlank() && expected.equalsIgnoreCase(providers.getString(key, "")))
        score += 20;
    }
    if (body.getValue("year") != null
        && String.valueOf(body.getValue("year"))
            .equals(String.valueOf(item.getValue("ProductionYear")))) score += 2;
    return score;
  }

  private Future<String> findControllableSession(String userId) {
    return webClient
        .getAbs(jellyfinBaseUrl + "/Sessions")
        .addQueryParam("ControllableByUserId", userId)
        .putHeader("authorization", buildJellyfinAuthorizationHeader())
        .putHeader("X-Emby-Token", jellyfinApiKey)
        .putHeader("X-MediaBrowser-Token", jellyfinApiKey)
        .putHeader("X-Api-Key", jellyfinApiKey)
        ...
        .compose(
            response -> {
              if (response.statusCode() < 200 || response.statusCode() >= 300)
                return Future.failedFuture("failed to find the active player");
              JsonObject latest =
                      ...
                      .filter(
                          session ->
                              session.getString("Id") != null
                                  && session.getBoolean("SupportsRemoteControl", true))
                      .max(
                          Comparator.comparing(
                              session -> session.getString("LastActivityDate", "")))
                      .orElse(null);
              return latest == null
                  ? Future.failedFuture("Open the Watch tab once to prepare the MovieHub player")
                  : Future.succeededFuture(latest.getString("Id"));
            });
  }
  ...
  public void handleCreateAccessRequest(RoutingContext context) {
    ...
    try {
      ...
      if (body == null) {
        ...
        Utility.buildResponse(
            context, 400, Utility.createErrorResponse("request body is required"));
        return;
      }
      ...
      if (!isValidMovieHubUsername(normalizedUsername)) {
        log.warn(
            "Create moviehub access request failed: invalid username format userId={} username={}",
            userId,
            normalizedUsername);
        Utility.buildResponse(
            context,
      ...
      log.info(
          "Creating moviehub access request userId={} username={}", userId, normalizedUsername);
      fetchUserById(userId)
          .compose(
              user -> {
                String email = user.getString("email", "").trim();
          ...
          .onSuccess(
              result -> {
                ...
                if (existingAccess != null) {
                  log.warn(
                      "Create moviehub access request conflict: access already approved userId={}",
                      userId);
                  Utility.buildResponse(
                      context,
                      409,
                      Utility.createErrorResponse(
                          "moviehub access is already approved for this user"));
                  return;
                }
                if (existingPendingRequest != null) {
                  log.warn(
                      "Create moviehub access request conflict: pending request already exists userId={}",
                      userId);
                  Utility.buildResponse(
                      context,
                      409,
                      Utility.createErrorResponse(
                          "moviehub access request is already pending approval"));
                  return;
                }
                if (usernameTaken) {
                  log.warn(
                      "Create moviehub access request conflict: username already in use userId={} username={}",
                      userId,
                      normalizedUsername);
                  Utility.buildResponse(
                      context,
                ...
                try {
                  ...
                  log.error(
                      "Failed to encrypt moviehub request password userId={}",
                      userId,
                      encryptionFail);
                  Utility.buildResponse(
                      context, 500, Utility.createErrorResponse("failed to secure password"));
                  return;
                }
                ...
                mongoDBClient
                    .insertRecord(
                        JsonObject.mapFrom(accessRequest), MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
                    .onSuccess(
                        inserted -> {
                          log.info(
                              "Created moviehub access request requestId={} userId={} username={}",
                          ...
                          Utility.buildResponse(
                              ...
                              Utility.createSuccessResponse(
                                  new JsonObject()
                                      ...
                                      .put(
                                          "movieHubUserName",
                                          accessRequest.getMovieHubUserName())));
                        })
                    .onFailure(
                        fail -> {
                          String errorMessage =
                              resolveErrorMessage(fail, "failed to create moviehub access request");
                          log.error(
                              "Failed to create moviehub access request userId={} username={}",
                              userId,
                              normalizedUsername,
                              fail);
                          Utility.buildResponse(
                              context, 500, Utility.createErrorResponse(errorMessage));
                        });
              })
          .onFailure(
              fail -> {
                String errorMessage =
                    resolveErrorMessage(fail, "failed to validate moviehub access request");
                log.error(
                    "Failed to validate moviehub access request for userId={} username={}",
                    userId,
                    normalizedUsername,
                    fail);
                Utility.buildResponse(context, 500, Utility.createErrorResponse(errorMessage));
      ...
      log.error(
          "Unexpected error while parsing moviehub access request userId={}", userId, exception);
  ...
  public void handleGetAccessRequests(RoutingContext context) {
    ...
    if (statusFilter != null && !statusFilter.isBlank()) {
      try {
        MovieHubAccessStatus status =
            MovieHubAccessStatus.valueOf(statusFilter.trim().toUpperCase());
    ...
    mongoDBClient
        .queryRecords(query, MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
        .onSuccess(
            records -> {
              List<JsonObject> sorted =
                  records.stream()
                      ...
                      .sorted(
                          Comparator.comparing(this::extractCreatedAt, Comparator.reverseOrder()))
                      .toList();
              log.info(
                  "Fetched moviehub access requests count={} statusFilter={}",
                  sorted.size(),
                  statusFilter);
        ...
        .onFailure(
            fail -> {
              String errorMessage =
                  resolveErrorMessage(fail, "failed to fetch moviehub access requests");
              log.error(
                  "Failed to fetch moviehub access requests statusFilter={}", statusFilter, fail);
              Utility.buildResponse(context, 500, Utility.createErrorResponse(errorMessage));
            });
  }
  ...
  public void handleApproveAccessRequest(RoutingContext context) {
    ...
    if (requestId == null || requestId.isBlank()) {
      log.warn(
          "Approve moviehub access request failed: missing requestId adminUserId={}", adminUserId);
      Utility.buildResponse(context, 400, Utility.createErrorResponse("requestId is required"));
      return;
    }
    log.info(
        "Approving moviehub access request requestId={} adminUserId={}", requestId, adminUserId);
    ...
    fetchAccessRequestById(requestId)
        .compose(
            request -> {
              ...
              return CompositeFuture.all(
                      ...
                      isMovieHubUsernameTaken(
                          request.getMovieHubUserNameLower(), request.getRequestId(), false))
                  .compose(
                      result -> {
                        JsonObject existingAccess = result.resultAt(0);
                                ...
                                jellyfinUser -> {
                                  ...
                                  return enforceJellyfinLimitedLibraryAccess(jellyfinUserId)
                                      .compose(
                                          v ->
                                              createAccessMapping(
                                                  request, adminUserId, jellyfinUserId))
                                      .compose(
                                          v ->
                                      ...
                                      .compose(
                                          v ->
                                              sendMovieHubCredentialsEmail(
                                                  request, decryptedPassword))
                                      .compose(v -> markCredentialsSent(request.getRequestId()))
                                      .map(
                                          new JsonObject()
                                              .put(
                                                  "message",
                                                  "moviehub access approved and jellyfin user created")
                                              .put("requestId", request.getRequestId())
                                              .put(
                                                  "movieHubUserName", request.getMovieHubUserName())
        ...
        .onSuccess(
            response -> {
              log.info(
                  "Approved moviehub access request requestId={} adminUserId={} notification={}",
                  requestId,
                  adminUserId,
                  response.getString("notification", "unknown"));
              Utility.buildResponse(context, 200, Utility.createSuccessResponse(response));
            })
        .onFailure(
            fail -> {
  ...
  public void handleRejectAccessRequest(RoutingContext context) {
    ...
    if (requestId == null || requestId.isBlank()) {
      log.warn(
          "Reject moviehub access request failed: missing requestId adminUserId={}", adminUserId);
      Utility.buildResponse(context, 400, Utility.createErrorResponse("requestId is required"));
      return;
    }
    log.info(
        "Rejecting moviehub access request requestId={} adminUserId={}", requestId, adminUserId);
    ...
    fetchAccessRequestById(requestId)
        .compose(
            request -> {
              ...
              return mongoDBClient
                  .updateRecord(query, update, MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
                  .compose(
                      v ->
                          sendMovieHubAccessRejectedEmail(request)
                              .map(
                                  new JsonObject()
                              ...
                              .recover(
                                  mailFail -> {
                                    log.error(
                                        "Failed to send rejection email for moviehub access requestId={}",
                                        requestId,
                                        mailFail);
                                    return Future.succeededFuture(
                                        new JsonObject()
        ...
        .onSuccess(
            response -> {
              log.info(
                  "Rejected moviehub access request requestId={} adminUserId={} notification={}",
                  requestId,
                  adminUserId,
                  response.getString("notification", "unknown"));
              Utility.buildResponse(context, 200, Utility.createSuccessResponse(response));
  ...
  public void handleGetAccessUsers(RoutingContext context) {
    ...
    mongoDBClient
        .queryRecords(query, MOVIEHUB_ACCESS_USERS_COLLECTION)
        .compose(
            records -> {
              List<JsonObject> sanitizedRecords =
                  records.stream().map(this::sanitizeRecord).toList();
              Set<String> userIds =
                  sanitizedRecords.stream()
                  ...
                  .map(
                      roleByUserId ->
                          sanitizedRecords.stream()
                              ...
                              .sorted(
                                  Comparator.comparing(
                                      this::extractCreatedAt, Comparator.reverseOrder()))
                              .toList());
            })
        .onSuccess(
            users -> {
              log.info(
                  "Fetched moviehub access users count={} adminUserId={}",
                  users.size(),
                  adminUserId);
    ...
    fetchAccessUserByMappingId(mappingId)
        .compose(
            accessUser -> {
              ...
              return fetchUserRoleByUserId(targetUserId)
                  .compose(
                      role -> {
                        if (Role.ADMIN.name().equalsIgnoreCase(role)) {
                        ...
                        return deleteJellyfinUser(jellyfinUserId)
                            ...
                            .map(
                                new JsonObject()
                                    ...
                                    .put(
                                        "movieHubUserName",
                                        accessUser.getString("movieHubUserName", ""))
        ...
        .onSuccess(
            response -> {
              log.info(
                  "Deleted moviehub access user mappingId={} adminUserId={}",
                  mappingId,
                  adminUserId);
              Utility.buildResponse(context, 200, Utility.createSuccessResponse(response));
            })
        .onFailure(
            fail -> {
              ...
              int statusCode =
                  message.toLowerCase().contains("not found")
                      ? 404
                      : (message.toLowerCase().contains("admin users cannot be deleted"))
                          ? 403
                          : (message.toLowerCase().contains("required")
                              ...
                              : 500;
              log.error(
                  "Failed to delete moviehub access user mappingId={} adminUserId={}",
                  mappingId,
                  adminUserId,
                  fail);
              Utility.buildResponse(context, statusCode, Utility.createErrorResponse(message));
        ...
        .compose(
            folders -> {
              ...
              if (allowedFolderIds.isEmpty()) {
                log.warn(
                    "Jellyfin limited library access failed: Movies/TV Shows folders not found userId={}",
                    jellyfinUserId);
                return Future.failedFuture(
                    "failed to locate Movies/TV Shows libraries in Jellyfin");
              }
                  ...
                  .compose(
                      existingPolicy ->
                          updateJellyfinUserPolicy(
                              jellyfinUserId, existingPolicy, allowedFolderIds, blockedFolderIds));
            })
        .onSuccess(
            v -> log.info("Applied Jellyfin limited library access userId={}", jellyfinUserId))
        .onFailure(
            fail ->
                log.error(
                    "Failed to enforce Jellyfin limited library access userId={}",
                    jellyfinUserId,
                    fail));
  }
  ...
  private Future<JsonArray> fetchJellyfinVirtualFolders() {
    ...
    webClient
        .getAbs(jellyfinBaseUrl + "/Library/VirtualFolders")
        ...
        .onSuccess(
            response -> {
              if (response.statusCode() < 200 || response.statusCode() >= 300) {
                ...
                promise.fail(
                    "failed to fetch jellyfin libraries (status="
                        + response.statusCode()
                        + ", body="
                        + safeBody
                        + ")");
  ...
  private JsonArray resolveAllowedFolderIds(JsonArray folders) {
    ...
    for (Object folderObj : folders) {
      ...
      if (tvShowsId == null
          && ("tv shows".equals(normalizedName) || "tvshows".equals(normalizedName))) {
        tvShowsId = itemId;
      }
    }
    if (moviesId == null || tvShowsId == null) {
      log.warn(
          "Jellyfin exact folder matching incomplete hasMovies={} hasTvShows={}",
          moviesId != null,
          tvShowsId != null);
      return new JsonArray();
    }
    JsonArray allowedFolderIds = new JsonArray().add(moviesId).add(tvShowsId);
    log.info(
        "Resolved Jellyfin allowed folders for Movies/TV Shows count={}", allowedFolderIds.size());
  ...
  private Future<JsonObject> fetchJellyfinUserPolicy(String jellyfinUserId) {
    ...
    webClient
        .getAbs(jellyfinBaseUrl + "/Users/" + jellyfinUserId)
        .putHeader("accept", "application/json")
        .putHeader("authorization", buildJellyfinAuthorizationHeader())
        .putHeader("X-Emby-Token", jellyfinApiKey)
        .putHeader("X-MediaBrowser-Token", jellyfinApiKey)
        ...
        .onSuccess(
            response -> {
              if (response.statusCode() < 200 || response.statusCode() >= 300) {
                String safeBody = truncateResponseBody(response.bodyAsString());
                promise.fail(
                    "failed to fetch jellyfin user policy (status="
                        + response.statusCode()
                        + ", body="
                        + safeBody
                        + ")");
              ...
              log.info(
                  "Fetched Jellyfin user policy userId={} hasPolicy={}",
                  jellyfinUserId,
                  policy != null);
  ...
  private Future<Void> updateJellyfinUserPolicy(
    ...
    webClient
        .postAbs(jellyfinBaseUrl + "/Users/" + jellyfinUserId + "/Policy")
        .putHeader("accept", "application/json")
        ...
        .onSuccess(
            response -> {
              if (response.statusCode() < 200 || response.statusCode() >= 300) {
                String safeBody = truncateResponseBody(response.bodyAsString());
                promise.fail(
                    "failed to update jellyfin user policy (status="
                        + response.statusCode()
                        + ", body="
                        + safeBody
                        + ")");
              ...
              log.info(
                  "Updated Jellyfin user policy userId={} enabledFolders={} blockedFolders={}",
                  jellyfinUserId,
                  enabledFolders.size(),
                  blockedFolders.size());
              promise.complete();
  ...
  public void handleResendTemporaryPassword(RoutingContext context) {
    ...
    fetchUserById(userId)
              ...
              return CompositeFuture.all(
                  ...
                  .compose(
                      result -> {
                        ...
                        if (accessUser == null) {
                          return Future.failedFuture(
                              "moviehub access is not approved for this user");
                        }
                        ...
                        try {
                          decryptedPassword =
                              decryptPassword(approvedRequest.getEncryptedPassword());
                        } catch (Exception decryptFail) {
                          return Future.failedFuture("failed to decrypt temporary password");
                        }
                        approvedRequest.setMovieHubUserName(
                            accessUser.getString(
                                "movieHubUserName", approvedRequest.getMovieHubUserName()));
        ...
        .onSuccess(
            response -> {
              log.info(
                  "Resent moviehub temporary password userId={} username={}",
                  userId,
  ...
  public void handleConfirmPasswordReset(RoutingContext context) {
    ...
    fetchUserById(userId)
        ...
        .onFailure(
            fail -> {
              String message =
                  resolveErrorMessage(fail, "failed to mark password reset confirmation");
              int statusCode =
                  message.toLowerCase().contains("approved")
                          || message.toLowerCase().contains("missing")
                          || message.toLowerCase().contains("invalid")
                  ...
                  .map(
                      result -> {
                        ...
                        if (approvedAccess != null) {
                          boolean showTemporaryPasswordNotice =
                              approvedAccess.getValue("passwordResetConfirmedAt") == null;
                          return new JsonObject()
                              .put("hasAccess", true)
                              .put("status", MovieHubAccessStatus.APPROVED.name())
                              .put("exists", true)
                        ...
                        if (latestRequest != null) {
                          return new JsonObject()
                              ...
                              .put(
                                  "status",
                                  latestRequest.getString(
                                      "status", MovieHubAccessStatus.PENDING.name()))
  ...
  private Future<JsonObject> fetchAccessUserByEmail(String email) {
    ...
    JsonObject query = new JsonObject().put("userEmail", email).put("active", true);
    Promise<JsonObject> promise = Promise.promise();
    mongoDBClient
        .queryRecords(query, MOVIEHUB_ACCESS_USERS_COLLECTION)
        .onSuccess(
            records ->
                promise.complete(
                    records == null || records.isEmpty() ? null : sanitizeRecord(records.get(0))))
        .onFailure(fail -> promise.fail(fail.getMessage()));
    return promise.future();
  }
  ...
  private Future<JsonObject> fetchUserRolesByUserIds(Set<String> userIds) {
    ...
    mongoDBClient
        .queryRecords(query, USER_COLLECTION)
        .onSuccess(
            records -> {
  ...
  private Future<String> fetchUserRoleByUserId(String userId) {
    ...
    mongoDBClient
        .queryRecords(new JsonObject().put("userId", userId), USER_COLLECTION)
        .onSuccess(
            records -> {
              if (records == null || records.isEmpty()) {
  ...
  private Future<JsonObject> fetchPendingAccessRequestForEmail(String email) {
    JsonObject query =
        new JsonObject().put("userEmail", email).put("status", MovieHubAccessStatus.PENDING.name());
    Promise<JsonObject> promise = Promise.promise();
    mongoDBClient
        .queryRecords(query, MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
        .onSuccess(
  ...
  private Future<JsonObject> fetchLatestRequestForEmail(String email) {
    Promise<JsonObject> promise = Promise.promise();
    mongoDBClient
        .queryRecords(new JsonObject().put("userEmail", email), MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
        .onSuccess(
            records -> {
              if (records == null || records.isEmpty()) {
                promise.complete(null);
    ...
    mongoDBClient
        ...
        .onSuccess(
            records -> {
              ...
              JsonObject latest =
                  records.stream().max(Comparator.comparing(this::extractCreatedAt)).orElse(null);
              if (latest == null) {
                promise.complete(null);
                return;
              }
  ...
  private Future<Boolean> isMovieHubUsernameTaken(
      String normalizedUsernameLower, String excludeRequestId, boolean includeJellyfinUsers) {
    Future<List<JsonObject>> activeUsersFuture =
        mongoDBClient.queryRecords(
            new JsonObject()
                .put("movieHubUserNameLower", normalizedUsernameLower)
                .put("active", true),
            MOVIEHUB_ACCESS_USERS_COLLECTION);
    JsonObject pendingQuery =
        new JsonObject()
            .put("movieHubUserNameLower", normalizedUsernameLower)
    ...
    Future<List<JsonObject>> pendingRequestsFuture =
        mongoDBClient.queryRecords(pendingQuery, MOVIEHUB_ACCESS_REQUESTS_COLLECTION);
    Future<JsonObject> jellyfinUserFuture =
        includeJellyfinUsers
  ...
  private Future<MovieHubAccessRequest> fetchAccessRequestById(String requestId) {
    ...
    mongoDBClient
        .queryRecords(
            new JsonObject().put("requestId", requestId), MOVIEHUB_ACCESS_REQUESTS_COLLECTION)
        .onSuccess(
            records -> {
              if (records == null || records.isEmpty()) {
  ...
  private Future<Void> createAccessMapping(
      MovieHubAccessRequest request, String adminUserId, String jellyfinUserId) {
    Instant now = Instant.now();
    ...
    return mongoDBClient.insertRecord(
        JsonObject.mapFrom(accessUser), MOVIEHUB_ACCESS_USERS_COLLECTION);
  }

  private Future<Void> markAccessRequestApproved(
      MovieHubAccessRequest request, String adminUserId, String jellyfinUserId) {
  ...
  private Future<Void> markCredentialsSent(String requestId) {
    ...
    JsonObject update =
        new JsonObject()
            .put("$set", new JsonObject().put("credentialsSentAt", now).put("updatedAt", now));
    return mongoDBClient.updateRecord(query, update, MOVIEHUB_ACCESS_REQUESTS_COLLECTION);
  }
  ...
  private Future<Void> markPasswordResetConfirmed(String mappingId) {
    ...
    JsonObject update =
        new JsonObject()
            .put(
                "$set",
                new JsonObject().put("passwordResetConfirmedAt", now).put("updatedAt", now));
  ...
  private Future<Void> clearPasswordResetConfirmationByEmail(String email) {
    ...
    JsonObject update =
        new JsonObject()
            .put(
                "$set",
                new JsonObject().put("passwordResetConfirmedAt", null).put("updatedAt", now));
    return mongoDBClient.updateRecord(query, update, MOVIEHUB_ACCESS_USERS_COLLECTION);
  }
  ...
  private Future<JsonObject> createJellyfinUser(String username, String password) {
    ...
    Promise<JsonObject> promise = Promise.promise();
    JsonObject payload = new JsonObject().put("Name", username).put("Password", password);
    webClient
        .postAbs(jellyfinBaseUrl + "/Users/New")
        .putHeader("accept", "application/json")
        .putHeader("Content-Type", "application/json")
        .putHeader("authorization", buildJellyfinAuthorizationHeader())
        .putHeader("X-Emby-Token", jellyfinApiKey)
            ...
            createResponse -> {
              if (createResponse.statusCode() < 200 || createResponse.statusCode() >= 300) {
                ...
                if (isJellyfinUserAlreadyExists(createResponse.statusCode(), responseBody)) {
                  log.info(
                      "Jellyfin user already exists username={}. Reusing existing user.", username);
                  fetchJellyfinUserByName(username)
                      .onSuccess(
                          existingUser -> {
                            if (existingUser == null) {
                              promise.fail(
                                  "jellyfin reported existing user but user lookup failed");
                              return;
                            }
                            log.info(
                                "Resolved existing Jellyfin user username={} userId={}",
                                username,
                                existingUser.getString("Id", ""));
                ...
                promise.fail(
                    "failed to create jellyfin user (status="
              ...
              if (createdUser != null && !createdUser.getString("Id", "").isBlank()) {
                log.info(
                    "Created Jellyfin user username={} userId={}",
                    username,
              ...
              fetchJellyfinUserByName(username)
                  .onSuccess(
                      existingUser -> {
                        ...
                        log.info(
                            "Resolved Jellyfin user via fallback username={} userId={}",
                            username,
                            existingUser.getString("Id", ""));
                        promise.complete(existingUser);
                      })
        ...
        .onSuccess(
            usersResponse -> {
              if (usersResponse.statusCode() < 200 || usersResponse.statusCode() >= 300) {
                ...
                promise.fail(
                    "failed to fetch jellyfin users (status="
                ...
                for (Object userObj : users) {
                  if (userObj instanceof JsonObject userJson) {
                    if (username.equalsIgnoreCase(userJson.getString("Name", ""))) {
                      log.info(
                          "Found Jellyfin user by username={} userId={}",
                          username,
  ...
  private Future<Void> deleteJellyfinUser(String jellyfinUserId) {
    ...
    Promise<Void> promise = Promise.promise();
    webClient
        .deleteAbs(jellyfinBaseUrl + "/Users/" + jellyfinUserId)
        .putHeader("accept", "*/*")
        .putHeader("authorization", buildJellyfinAuthorizationHeader())
        .putHeader("X-Emby-Token", jellyfinApiKey)
        .putHeader("X-MediaBrowser-Token", jellyfinApiKey)
        ...
        .onSuccess(
            response -> {
              ...
              if (response.statusCode() < 200 || response.statusCode() >= 300) {
                String safeBody = truncateResponseBody(response.bodyAsString());
                promise.fail(
                    "failed to delete jellyfin user (status="
                        + response.statusCode()
                        + ", body="
                        + safeBody
                        + ")");
  ...
  private Future<Void> sendMovieHubCredentialsEmail(
      MovieHubAccessRequest request, String userPassword) {
    if (request.getUserEmail() == null || request.getUserEmail().isBlank()) {
      return Future.failedFuture("request email missing");
    }
    log.info(
        "Sending MovieHub approval credentials email requestId={} email={}",
        request.getRequestId(),
        request.getUserEmail());
    String subject = "MovieHub access approved";
    String htmlBody = buildMovieHubAccessEmail(request, userPassword);
    return new MailService(webClient)
        .sendEmail(subject, request.getUserEmail(), htmlBody)
        .onSuccess(
            v ->
                log.info(
                    "Sent MovieHub approval credentials email requestId={}",
                    request.getRequestId()))
        .onFailure(
            fail ->
                log.error(
                    "Failed to send MovieHub approval credentials email requestId={}",
                    request.getRequestId(),
                    fail));
  }
  ...
  private Future<Void> sendMovieHubAccessRejectedEmail(MovieHubAccessRequest request) {
    ...
    log.info(
        "Sending MovieHub rejection email requestId={} email={}",
        request.getRequestId(),
        request.getUserEmail());
    ...
    return new MailService(webClient)
        .sendEmail(subject, request.getUserEmail(), htmlBody)
        .onSuccess(
            v -> log.info("Sent MovieHub rejection email requestId={}", request.getRequestId()))
        .onFailure(
            fail ->
                log.error(
                    "Failed to send MovieHub rejection email requestId={}",
                    request.getRequestId(),
                    fail));
  }
    ...
    String safeName =
        escapeHtml(
            request.getUserName() == null || request.getUserName().isBlank()
                ...
                : request.getUserName());
    String safeUserName =
        escapeHtml(request.getMovieHubUserName() == null ? "" : request.getMovieHubUserName());
    return """
                <html>
                  <head>
                    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  ...
  private String encryptPassword(String plainTextPassword) throws GeneralSecurityException {
    ...
    byte[] key =
        MessageDigest.getInstance("SHA-256").digest(cryptoSecret.getBytes(StandardCharsets.UTF_8));
    byte[] iv = new byte[12];
