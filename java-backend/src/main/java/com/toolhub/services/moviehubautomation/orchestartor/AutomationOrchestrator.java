...
public class AutomationOrchestrator {
  ...
  private AutomationOrchestrator(IntentStrategyFactory strategyFactory, AiClient aiClient) {
  ...
  public Future<IntentResponse> orchestrate(String userInput, ConversationContext context) {
    ...
    log.info("Orchestrating conversationId={}, input={}", context.getConversationId(), userInput);
    ...
    if (Intent.UNKNOWN.equals(intent)) {
      ...
      if (!Intent.UNKNOWN.equals(regexIntent)) {
        log.info(
            "Regex resolved intent={} for conversationId={}",
            regexIntent,
          ...
          .onSuccess(
              llmResponse -> {
                ...
                if (classifiedIntent == null || Intent.UNKNOWN.equals(classifiedIntent)) {
                  IntentResponse response = new IntentResponse();
                  response.setMessage(
                      "I’m not sure I understood that. You can ask me to download or request a movie/TV show.");
                  chatResponsePromise.complete(response);
                  return;
                }
  ...
  private void executeIntent(
      ConversationContext context, String userInput, Promise<IntentResponse> chatResponsePromise) {
    strategyFactory
  ...
  private void summarizeIfNeeded(
    ...
    JsonObject summaryRequest =
        OpenAiRequestBuilder.buildPayload(
            context.getMediaState(), backendResponse.getMessage(), Intent.SUMMARIZE);

    aiClient
        .makeAiCall(summaryRequest)
        .onSuccess(
            summaryResponse -> {
