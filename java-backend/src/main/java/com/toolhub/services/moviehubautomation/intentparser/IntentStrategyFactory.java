...
public class IntentStrategyFactory {
  ...
  public IntentStrategyFactory(List<IntentStrategy> strategyList) {
    this.strategies =
        strategyList.stream()
            .collect(Collectors.toMap(IntentStrategy::getIntent, Function.identity()));
    log.debug(
        "IntentStrategyFactory initialized with {} strategies: {}",
        strategies.size(),
        strategies.keySet());
  }
  ...
  public IntentStrategy getStrategy(Intent intent) {
    ...
    if (strategy == null) {
      ...
      throw new IllegalStateException("No strategy registered for intent: " + intent);
    }
