...
public class AddMediaClient {
  ...
  public static Future<JsonObject> add(
      WebClient webClient, String addMovieUrl, String apiKey, AddMediaPayload addMediaPayload) {
    ...
    try {
      ...
      client
          ...
          .onSuccess(
              res -> {
                if (res.statusCode() >= 200 && res.statusCode() < 300) {
                  ...
                  log.warn(
                      "Add movie request returned non-success status {} for title={}",
                      res.statusCode(),
                      addMediaPayload.getTitle());
          ...
          .onFailure(
              fail -> {
                log.error(
                    "Add movie request failed for title={}: {}",
                    addMediaPayload.getTitle(),
                    fail.getMessage());
