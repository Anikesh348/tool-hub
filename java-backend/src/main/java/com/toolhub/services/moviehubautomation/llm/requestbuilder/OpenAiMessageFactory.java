...
public class OpenAiMessageFactory {
  ...
  private static final Map<Intent, BaseOpenAiMessageBuilder> INTENT_TO_MESSAGE_BUILDER =
      Map.of(
  ...
  public static BaseOpenAiMessageBuilder build(Intent intent) {
    ...
    log.info(
        "Returning message builder: {} for intent: {}", builder.getClass().getSimpleName(), intent);
    return builder;
  }
}
