...
public class OpenAiRequestBuilder {
  ...
  public static JsonObject buildPayload(MediaState mediaState, String input, Intent intent) {
    ...
    JsonObject request =
        new JsonObject().put("model", "gpt-4o-mini").put("temperature", 0).put("max_tokens", 200);
    JsonArray messages =
        new JsonArray()
            .add(systemMessage())
            .add(OpenAiMessageFactory.build(intent).buildMessage(mediaState, input));
  ...
  private static JsonObject systemMessage() {
    return new JsonObject().put("role", "system").put("content", Templates.SYSTEM_PROMPT);
  }
}
