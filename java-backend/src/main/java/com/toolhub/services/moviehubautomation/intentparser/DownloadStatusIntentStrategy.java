...
public class DownloadStatusIntentStrategy implements IntentStrategy {
  ...
  public Future<IntentResponse> automate(ConversationContext context, String userInput) {
    ...
    JsonObject parseRequest =
        OpenAiRequestBuilder.buildPayload(context.getMediaState(), userInput, supportedIntent);
    aiClient
        ...
        .onSuccess(
            llmResponse -> {
              ...
              Future<JsonObject> activeDownloadsFuture =
                  movieHubRequestPortalService.getDownloadQueue(
                      context.getUserId(), context.getUserRole(), scope);
              Future<JsonObject> completedDownloadsFuture =
                  movieHubRequestPortalService.getCompletedDownloads(
                      context.getUserId(), context.getUserRole(), scope);
              ...
              CompositeFuture.all(activeDownloadsFuture, completedDownloadsFuture)
                  .onSuccess(
                      result -> {
                        ...
                        JsonArray activeDownloads =
                            activePayload.getJsonArray("downloads", new JsonArray());
                        JsonArray completedDownloads =
                            completedPayload.getJsonArray("downloads", new JsonArray());
                        JsonArray filteredActive =
                            filterDownloads(activeDownloads, titleFilter, mediaTypeFilter);
                        JsonArray filteredCompleted =
                            filterDownloads(completedDownloads, titleFilter, mediaTypeFilter);
                  ...
                  .onFailure(
                      fail -> {
                        IntentResponse response = new IntentResponse();
                        response.setMessage(
                            "Failed to fetch download status: " + fail.getMessage());
                        promise.complete(response);
                      });
            })
        .onFailure(
  ...
  private JsonArray filterDownloads(
      JsonArray downloads, String titleFilter, MediaType mediaTypeFilter) {
  ...
  private String buildQueueSummary(
    ...
    if (!activeDownloads.isEmpty()) {
      ...
      for (int i = 0; i < activeLimit; i++) {
        ...
        if (progress instanceof Number number) {
          builder
              .append(" | progress=")
              .append(String.format(Locale.ROOT, "%.1f%%", number.doubleValue()));
        }
      ...
      if (activeDownloads.size() > activeLimit) {
        builder
            .append("Additional active entries not shown: ")
    ...
    if (!completedDownloads.isEmpty()) {
      ...
      if (completedDownloads.size() > completedLimit) {
        builder
            .append("Additional completed entries not shown: ")
