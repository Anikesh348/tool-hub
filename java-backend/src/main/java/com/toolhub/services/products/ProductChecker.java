...
public class ProductChecker {
  ...
  public void checkAllProducts() {
    mongoDBClient
        .queryRecords(new JsonObject(), "products")
        .onSuccess(
            productList -> {
              List<Product> products =
                  productList.stream()
                      .map(productJson -> castToClass(productJson, Product.class))
                      .toList();
              BatchProcessor<Product> batchProcessor =
                  new SaveHistoryAndAlertBatchProcessor(
                      mongoDBClient, vertx, scrapperClient, client);
