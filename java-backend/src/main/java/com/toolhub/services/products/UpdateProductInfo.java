...
public class UpdateProductInfo {
  ...
  public void updateProductInfo(Product product) {
    scrapperClient
        .getScrappedProductDetails(product)
        .compose(
            response -> {
  ...
  private Future<Void> saveProductInfo(ProductInfo productInfo) {
    return mongoDBClient
        .insertRecord(JsonObject.mapFrom(productInfo), "product-information")
        .onSuccess(
            res -> log.info("Inserted product information for {}", productInfo.getProductId()))
        .onFailure(
            error ->
                log.error(
