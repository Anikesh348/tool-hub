...
import com.toolhub.models.ProductInfo;
...
public class GetProducts {
  ...
  private void fetchProducts() {
    JsonObject userQuery =
        new JsonObject()
            .put(
                "userTargetPrices",
                new JsonObject().put("$elemMatch", new JsonObject().put("userId", userId)));
    mongoDBClient
        .queryRecords(userQuery, "products")
  ...
  private List<String> extractProductIds(List<JsonObject> productList) {
    return productList.stream().map(product -> product.getString("productId")).distinct().toList();
  }

  private void fetchProductInformation(List<String> productIds) {
    JsonObject query =
        new JsonObject().put("productId", new JsonObject().put("$in", new JsonArray(productIds)));

    mongoDBClient
        .queryRecords(query, "product-information")
  ...
  private void handleError(Throwable failure) {
    Utility.buildResponse(context, 500, Utility.createErrorResponse(failure.getMessage()));
  }
                      ...
                      if (userTargetPrice.getUserId().equals(userId)) {
                        userTargetPrice
                            ...
                            .forEach(
                                targetPrice ->
                                    finalResponse.add(
                                        createProductEntry(product, targetPrice, productInfo)));
                      }
                    });
          } catch (Exception e) {
  ...
  private JsonObject createProductEntry(
      Product product, String targetPrice, ProductInfo productInfo) {
