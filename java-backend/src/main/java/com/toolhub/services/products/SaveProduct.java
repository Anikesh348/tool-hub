...
import com.toolhub.Utils.UrlRedirectUtil;
import com.toolhub.Utils.Utility;
import com.toolhub.models.Product;
import com.toolhub.models.ProductStatus;
import com.toolhub.models.UserTargetPrices;
...
public class SaveProduct {
  ...
  public void saveProduct(RoutingContext context) {
    ...
    UrlRedirectUtil.finalUrl(vertx, productUrl)
        .onSuccess(
            url -> {
              ...
              queryForProductId(productId, userId, targetPrice)
                  .onSuccess(
                      productStatus -> {
                        if (!productStatus.isProductExists()) {
                          ...
                          mongoDBClient
                              .insertRecord(JsonObject.mapFrom(product), "products")
                              .onSuccess(
                                  res -> {
                                    buildResponse(
                                        context, 200, createSuccessResponse("product inserted"));
                                  })
                              .onFailure(
                                  fail ->
                                      buildResponse(
                          ...
                          if (productStatus.isUserExists()
                              && !productStatus.isTargetPriceExists()) {
                            query
                            ...
                            JsonObject newUserEntry =
                                new JsonObject()
                                    .put("userId", userId)
                                    .put("targetPrices", new JsonArray().add(targetPrice));
                            update.put(
                                "$push", new JsonObject().put("userTargetPrices", newUserEntry));
                          }
                          mongoDBClient
                  ...
                  .onFailure(
                      fail ->
                          buildResponse(
                              context, 500, createErrorResponse("error adding product, retry!!")));
            })
        .onFailure(
            fail -> {
  ...
  public Future<ProductStatus> queryForProductId(
      String productId, String userId, String targetPrice) {
    Promise<ProductStatus> promise = Promise.promise();
    ProductStatus productStatus = new ProductStatus();
    JsonObject query = new JsonObject().put("productId", productId);
    mongoDBClient
        .queryRecords(query, "products")
        .onSuccess(
            res -> {
              if (!res.isEmpty()) {
