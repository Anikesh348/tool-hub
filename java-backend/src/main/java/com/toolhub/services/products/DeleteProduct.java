...
public class DeleteProduct {
  ...
  private void removeTargetPrice(
      String productId, String userId, String targetPrice, RoutingContext context) {
    JsonObject query =
        new JsonObject().put("productId", productId).put("userTargetPrices.userId", userId);

    JsonObject update =
        new JsonObject()
            .put("$pull", new JsonObject().put("userTargetPrices.$.targetPrices", targetPrice));

    mongoDBClient
        .updateRecord(query, update, "products")
        .onSuccess(res -> removeUserIfNoPrices(productId, userId, context))
        .onFailure(
            handleFailureOrProceed(
                () -> removeUserIfNoPrices(productId, userId, context), context));
  }
  ...
  private void removeUserIfNoPrices(String productId, String userId, RoutingContext context) {
    ...
    JsonObject update =
        new JsonObject()
            .put(
                "$pull",
                new JsonObject().put("userTargetPrices", new JsonObject().put("userId", userId)));

    mongoDBClient
        .updateRecord(query, update, "products")
        .onSuccess(res -> deleteProductIfNoUsers(productId, context))
        .onFailure(
            handleFailureOrProceed(() -> deleteProductIfNoUsers(productId, context), context));
  }
  ...
  private void deleteProductIfNoUsers(String productId, RoutingContext context) {
    ...
    mongoDBClient
        .deleteRecord(query, "products")
        .onSuccess(
            res ->
                Utility.buildResponse(
                    context, 200, Utility.createSuccessResponse("Target price removed")))
        .onFailure(
            handleFailureOrProceed(
                () ->
                    Utility.buildResponse(
  ...
  private Handler<Throwable> handleFailureOrProceed(
      Runnable onNoMatchFallback, RoutingContext context) {
    return err -> {
  ...
  private void respondWithError(RoutingContext context, Throwable err) {
    Utility.buildResponse(
        context,
        500,
        Utility.createErrorResponse("Database operation failed: " + err.getMessage()));
  }
}
