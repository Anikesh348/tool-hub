...
I use the proxy for ordinary browser tools on selected subdomains. The production blog root remains direct, and some long-lived or large media responses are also easier to serve and debug that way. The current setup is deliberately mixed, so I treat proxying as a practical choice rather than a badge that one option is always safer.
