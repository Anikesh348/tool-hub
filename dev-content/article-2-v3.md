...
![The real request paths for the dev blog and a protected metrics page, from the browser through Cloudflare and Caddy to the Raspberry Pi](assets/diagrams/public-request-path-v4.svg)

*The blog takes the direct application path after Caddy chooses its hostname. The metrics route pauses at my authentication gateway and reaches the monitoring service only after an access decision.*
