...
The production blog is the more useful example. Every published article follows the same address pattern: `hostingfrompurva.xyz/blogs/<article-name>`.

![The journey of a production blog request from the browser through Cloudflare DNS, Caddy and WireGuard to ToolHub on the Raspberry Pi, then back to the browser](assets/diagrams/blog-request-journey-v2.svg)

*For the public blog route, Caddy does not pause for the authentication gateway. It matches the production hostname and sends the request through WireGuard to ToolHub on the Pi.*

1. Your browser asks [Cloudflare DNS](#term:cloudflare-dns) where `hostingfrompurva.xyz` lives.
2. The production root record returns the VPS address directly, so the browser establishes HTTPS with Caddy on that machine.
3. Caddy receives the hostname `hostingfrompurva.xyz` and matches it to the production ToolHub route.
4. That route crosses WireGuard to the ToolHub frontend on the Pi.
...
The active Caddy route is roughly this small:
...
metrics.hostingfrompurva.xyz {
    ...
    reverse_proxy 10.90.0.2:19999
}
```

`forward_auth` makes a small local request to the gateway before the original request is forwarded. An allowed result lets Caddy continue to the Pi; any other result becomes the response to the visitor.

![How Caddy on the VPS checks a protected metrics request with the local authentication gateway before allowing it through WireGuard to the Raspberry Pi](assets/diagrams/protected-request-auth-flow-v1.svg)

*The authentication check stays on the VPS. Only an allowed request crosses WireGuard to the metrics service on the Pi.*
