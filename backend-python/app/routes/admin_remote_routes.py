...
from app.services.host_admin import pi_host_admin_request
...
def pi5_render_status(_: Dict[str, str] = Depends(admin_user)):
    try:
        return success(pi_host_admin_request("GET", "/v1/pi5-render"))
...
def pause_pi5_render(_: Dict[str, str] = Depends(admin_user)):
    try:
        return success(pi_host_admin_request("POST", "/v1/pi5-render/pause", timeout=45))
    except RuntimeError as exc:
        return JSONResponse(status_code=503, content=error(str(exc)))
...
def resume_pi5_render(_: Dict[str, str] = Depends(admin_user)):
    try:
        return success(pi_host_admin_request("POST", "/v1/pi5-render/resume", timeout=45))
    except RuntimeError as exc:
        return JSONResponse(status_code=503, content=error(str(exc)))
...
def start_pi5_airplay(_: Dict[str, str] = Depends(admin_user)):
    try:
        return success(pi_host_admin_request("POST", "/v1/pi5-airplay/start", timeout=45))
    except RuntimeError as exc:
        return JSONResponse(status_code=503, content=error(str(exc)))
...
def stop_pi5_airplay(_: Dict[str, str] = Depends(admin_user)):
    try:
        return success(pi_host_admin_request("POST", "/v1/pi5-airplay/stop", timeout=45))
    except RuntimeError as exc:
        return JSONResponse(status_code=503, content=error(str(exc)))
