...
The design also validated itself in a second, unplanned way: the same gateway/executor pattern was independently reused by an unrelated system (the ops scheduler, via `codex-operator-gateway` and `claude-operator-gateway`) with its own client identity and its own limits, without any change to either provider's executor and without requiring a change to ToolHub's chat gateways. That is precisely the isolation property 2.2 predicted before either operator gateway existed — and, as 7.6a and Lesson 9 explain, the same pair is now also used by confirmed ToolHub smart jobs, still without relaxing chat's `knowledge-only` / `read-only` ceiling.
...
**Who calls it, and how failover works there too.** Two ToolHub-adjacent callers exist:

- **opsched** on `ubuntu-purva` (systemd timers) still calls `operator` for `docker_update`, `log_digest`, and `connectivity_alert`. Its `ai_client.py` independently reimplements preferred/fallback logic — same usage-exhaustion codes as `ai_provider_router.py` (1.7, 3.6) — but *without* a Redis pin: each job run tries providers fresh. That's reasonable because these jobs are infrequent and don't share the web backend's cross-worker pin problem.
- **ToolHub smart scheduled jobs** (`user_scheduler.py` → `run_operator_prompt`) call the same operator gateway pair after an admin **confirms** a draft from Scheduler AI. Chat still cannot request `operator` (1.5). The confirm step is the authorization boundary; the later cron tick is unattended.

opsched's `ai_client.py` historically tried Codex first. ToolHub chat routing is Claude-first as of the live router (1.7). Do not assume the two attempt orders stay identical forever — they share exhaustion *codes*, not necessarily `PREFERRED_PROVIDER`.
