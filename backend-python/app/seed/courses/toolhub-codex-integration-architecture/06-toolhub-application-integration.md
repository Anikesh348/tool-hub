...
Every chat/Q&A feature — `ai_chats.py` (central assistant), `courses.py` (course questions), `leetcode_ai.py`, `leetcode_set_wizard.py`, `moviehub_chat_routes.py`, and `scheduler_ai.py` — never calls `gateway_request` directly. They call `routed_gateway_request` from `ai_provider_router.py`, which returns `(provider, response)` instead of just `response`. Lesson 9 lists each caller’s profile. Confirmed smart jobs are the exception: they use `operator_gateway.py`, not this router. That returned `provider` is what gets persisted (6.3) and what a caller of `/v2/admin/ai/health` sees under `routing.active`. The router itself is described in full in 1.7 and 4.8; the two things worth repeating in an application context are:
