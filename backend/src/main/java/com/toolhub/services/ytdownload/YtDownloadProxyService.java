...
public class YtDownloadProxyService {
    ...
    public YtDownloadProxyService(WebClient webClient, Vertx vertx, MongoDBClient mongoDBClient, Dotenv dotenv) {
        ...
        mongoDBClient.queryRecords(duplicateCheckQuery, YT_DOWNLOADS_COLLECTION)
                .onSuccess(existingRequests -> {
                    ...
                    mongoDBClient.insertRecord(record, YT_DOWNLOADS_COLLECTION)
                            ...
                            .onFailure(fail -> {
                                log.error("Failed to add yt download request requestId={} videoId={}", requestId, videoId, fail);
                                buildResponse(context, 500, createErrorResponse("failed to add download request"));
                            });
                })
                .onFailure(fail -> {
                    log.error("Failed to check duplicate yt download request videoId={}", videoId, fail);
                    buildResponse(context, 500, createErrorResponse("failed to validate download request"));
                });
    }
    ...
    public void handleCheckAndUpdate(RoutingContext context) {
        ...
        fetchDownloadsForStatusCheck()
                ...
                .onFailure(fail -> {
                    ...
                    buildResponse(context, 500, createErrorResponse("failed to check yt download status"));
                });
    }

    public void handleListRequests(RoutingContext context) {
        mongoDBClient.queryRecords(new JsonObject(), YT_DOWNLOADS_COLLECTION)
                .onSuccess(records -> {
                    List<JsonObject> sorted = new ArrayList<>(records);
                    sorted.sort((a, b) -> b.getString("createdAt", "").compareTo(a.getString("createdAt", "")));
                    buildResponse(context, 200, createSuccessResponse(new JsonObject()
                            .put("requests", sorted)));
                })
                .onFailure(fail -> {
    ...
    private void startDownloadFromQueue(RoutingContext context) {
        ...
        checkIfDownloadRunning()
                .compose(running -> {
                    ...
                    return fetchNextRequestedDownload().compose(nextRequest -> {
                        if (nextRequest == null) {
                            return Future.succeededFuture(new JsonObject()
                                    .put("started", false)
                        ...
                        return triggerDownload(nextRequest)
                                .compose(startResponse -> {
                                    ...
                                    return updateDownloadRecord(requestId, update).map(new JsonObject()
                                            .put("started", true)
                                ...
                                .recover(fail -> {
                                    log.error("Failed to trigger yt download for requestId={}", requestId, fail);
                                    ...
                                    Future<Void> recordUpdate = requestId.isBlank()
                                            ? Future.succeededFuture()
                                            : updateDownloadRecord(requestId, update);
                                    return recordUpdate
                                            .recover(updateFail -> {
                                                log.error("Failed to reset request status after start failure requestId={}", requestId, updateFail);
    ...
    private void fetchNextRequestedDownloadFromQueue(Promise<JsonObject> promise, int inspectedCount) {
        String requestId = pendingQueue.pollFirst();
        if (requestId == null || requestId.isBlank()) {
            fetchOldestRequestedFromDb()
    ...
    private Future<JsonObject> fetchOldestRequestedFromDb() {
        Promise<JsonObject> promise = Promise.promise();
        mongoDBClient.queryRecords(new JsonObject().put("status", STATUS_REQUESTED), YT_DOWNLOADS_COLLECTION)
                .onSuccess(records -> {
    ...
    private Future<JsonObject> fetchDownloadByRequestId(String requestId) {
        ...
        mongoDBClient.queryRecords(new JsonObject().put("requestId", requestId), YT_DOWNLOADS_COLLECTION)
                .onSuccess(records -> {
                    if (records == null || records.isEmpty()) {
                        promise.complete(null);
                        return;
    ...
    private Future<List<JsonObject>> fetchDownloadsForStatusCheck() {
        JsonObject query = new JsonObject()
                ...
                .put("$or", new JsonArray()
                        ...
                        .add(new JsonObject().put("downloadAlertSent", new JsonObject().put("$exists", false))));
    ...
    private void processDownloadChecks(List<JsonObject> downloads, int idx, JsonObject summary, Promise<JsonObject> promise) {
    ...
    private Future<Void> processSingleDownloadCheck(JsonObject record, JsonObject summary) {
        ...
        return fetchDownloadStatus(videoId)
                ...
                .recover(fail -> {
                    log.error("Failed while checking status for requestId={} videoId={}", requestId, videoId, fail);
                    return Future.succeededFuture();
                });
    }
    ...
    private Future<Void> markDownloadedAndAlert(JsonObject record, JsonObject statusPayload, JsonObject summary) {
        String requestId = record.getString("requestId", "");
        ...
        if (userEmail.isBlank()) {
            ...
            sendMailFuture = sendDownloadedEmail(userEmail, title)
                    .map(true)
                    .recover(fail -> {
        ...
        Future<Boolean> refreshFuture = triggerMovieHubLibraryRefresh()
                .map(true)
                .recover(fail -> {
                    log.error("Failed to trigger moviehub library refresh for requestId={}", requestId, fail);
                    return Future.succeededFuture(false);
                });

        return CompositeFuture.all(sendMailFuture, refreshFuture).compose(result -> {
            ...
            if (refreshTriggered) {
                summary.put("movieHubRefreshTriggered", summary.getInteger("movieHubRefreshTriggered", 0) + 1);
            }
            JsonObject update = new JsonObject()
                    ...
                    .put("movieHubRefreshTriggeredAt", refreshTriggered ? Instant.now().toString() : null)
    ...
    private boolean isFailedStatus(JsonObject payload) {
        ...
        if (payload.containsKey("error") && payload.getString("error") != null && !payload.getString("error").isBlank()) {
            return true;
        }
