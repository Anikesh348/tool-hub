...
public class AddMediaIntentStrategy implements IntentStrategy {
    ...
    public Future<IntentResponse> automate(ConversationContext context, String userInput) {
        ...
        log.info("Processing intent={} (effectiveIntent={}) for conversationId={}, userInput={}",
                supportedIntent, effectiveIntent, context.getConversationId(), userInput);

        if (context.isAwaitingSelection()) {
            handleSelection(context, userInput, intentResponsePromise, effectiveIntent, downgradedToRequest);
        ...
        aiClient.makeAiCall(request).onSuccess(llmResponse -> {
            try {
                ...
                if (llmResponse.getClarification().isEmpty()) {
                    if (mediaState.getMediaType() == null || MediaType.UNKNOWN.equals(mediaState.getMediaType())) {
                        intentResponse.setMessage(withPermissionNotice(
                                "Should I fetch a movie or a show?",
                                downgradedToRequest
                        ));
                        intentResponsePromise.complete(intentResponse);
                        return;
                    }
                    movieHubRequestPortalService.getTopLookupOptions(mediaState.getTitle(), mediaState.getMediaType(), 5)
                            .onSuccess(options -> {
                                ...
                                log.error("Lookup options fetch failed for title={}", mediaState.getTitle(), fail);
                                intentResponse.setMessage(withPermissionNotice(
    ...
    private void finalizeAction(ConversationContext context, Promise<IntentResponse> intentResponsePromise, Intent effectiveIntent) {
        IntentResponse intentResponse = new IntentResponse();
        MediaState mediaState = context.getMediaState();
        LookUpDTO lookUpDTO = buildLookUpDTO(mediaState);

        if (Intent.DOWNLOAD_MEDIA.equals(effectiveIntent)) {
            addMediaControllerFactory.getClient(mediaState.getMediaType())
                    .addContent(lookUpDTO)
                    .onComplete(addContentRes -> {
                        if (addContentRes.succeeded()) {
                            log.info("Successfully queued {} for download. Creating tracking request for admin userId={}",
                                    mediaState.getTitle(), context.getUserId());
                            movieHubRequestPortalService.createApprovedRequestFromAutomation(context.getUserId(), lookUpDTO)
                                    .onSuccess(trackingRes -> {
                                        intentResponse.setMessage(mediaState.getTitle() + " queued for download");
                                        context.setCompleted(true);
                                        intentResponsePromise.complete(intentResponse);
                                    })
                                    .onFailure(trackingFail -> {
                                        log.error("Media queued but failed to create tracking request for title={} userId={}",
                                                mediaState.getTitle(), context.getUserId(), trackingFail);
                                        intentResponse.setMessage(mediaState.getTitle() + " queued for download, but tracking for completion alerts failed.");
                                        context.setCompleted(true);
                                        intentResponsePromise.complete(intentResponse);
                                    });
                        } else {
        ...
        movieHubRequestPortalService.createRequestFromAutomation(context.getUserId(), lookUpDTO)
                .onSuccess(res -> {
                    intentResponse.setMessage(mediaState.getTitle() + " request submitted for admin approval.");
                    context.setCompleted(true);
                    intentResponsePromise.complete(intentResponse);
                })
                .onFailure(fail -> {
