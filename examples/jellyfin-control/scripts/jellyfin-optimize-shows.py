...
import subprocess, json, os, time, errno, shlex, shutil
...
ROOT = Path("/srv/data/media/shows")
LOG_PREFIX = "[SHOWS]"
INDEX_FILE = Path("/srv/jellyfin-control/scripts/logs/converted-shows.jsonl")
PROGRESS_DIR = Path("/srv/jellyfin-control/scripts/logs/progress")
PROGRESS_FILE = PROGRESS_DIR / "shows.json"
...
SUPPORTED_AUDIO_CODECS = {TARGET_AUDIO}
...
def parse_show(path: Path):
    parts = path.relative_to(ROOT).parts
    show = parts[0] if len(parts) > 0 else "Unknown Show"
    season = parts[1] if len(parts) > 1 else "Unknown Season"
    return show, season, path.name
...
def matches_target_profile(p: Path):
    data = ffprobe(p)
    video_stream = next((s for s in data.get("streams", []) if s.get("codec_type") == "video"), None)
    return (
        video_stream_is_supported(video_stream, data)
        and all_audio_streams_supported(data)
        and not has_unsupported_subtitles(data)
    )
...
def all_audio_streams_supported(data):
    ...
    return all(audio_stream_is_supported(stream) for stream in audio_streams)
...
def publish_preconvert_progress(video_path: Path, phase: str, percent: float, total_duration=0.0, started_at=None):
    show, season, filename = parse_show(video_path)
    title = f"{show} / {season} / {filename}"
...
def record_existing_optimal(p: Path, index_cache: set):
    show, season, episode = parse_show(p)
    final_output = get_final_output_path(p)
    record_conversion({
        "show": show,
        "season": season,
        "episode": final_output.name,
        "file": p.name,
        "source_path": str(p),
...
def acquire_file_lock(p: Path):
    ...
    while True:
        try:
            ...
            show, season, episode = parse_show(p)
            log("DELETE_STALE_LOCK", f"{show} / {season} / {episode}")
            try:
                lock.unlink()
            except Exception as e:
                log("DELETE_STALE_LOCK_FAILED", f"{show} / {season} / {episode}", str(e))
                return None
            continue
...
def should_skip_existing_output(p: Path, index_cache: set):
    ...
    if partial_output != p and partial_output.exists():
        show, season, episode = parse_show(p)
        if lock_is_active(p):
            log("SKIP_ACTIVE_TEMP", f"{show} / {season} / {episode}", str(partial_output))
            return True
        log("DELETE_STALE_TEMP", f"{show} / {season} / {episode}", str(partial_output))
    ...
    if final_output == p and is_indexed_media(p, index_cache):
        ...
        try:
            if is_low_cpu_cost(p):
                log("SKIP_INDEXED_OUTPUT", f"{show} / {season} / {episode}", "already converted")
                return True
            log("RECHECK_INDEX", f"{show} / {season} / {episode}", "file no longer compliant")
        except Exception as e:
            log("RECHECK_INDEX_FAILED", f"{show} / {season} / {episode}", str(e))
    ...
    if final_output.exists():
        ...
        try:
            if is_low_cpu_cost(final_output):
                log("SKIP_EXISTING_OUTPUT", f"{show} / {season} / {episode}", str(final_output))
                add_index_path_variants(index_cache, final_output)
                add_index_path_variants(index_cache, p)
                return True
            log("RECONVERT_EXISTING_OUTPUT", f"{show} / {season} / {episode}", "existing output is not compliant")
            index_cache.discard(str(p))
            index_cache.discard(str(final_output))
            return False
        except Exception as e:
            log("RECHECK_EXISTING_OUTPUT_FAILED", f"{show} / {season} / {episode}", str(e))
            index_cache.discard(str(p))
            index_cache.discard(str(final_output))
            return False
...
def is_low_cpu_cost(p):
    ...
    if not all_audio_streams_supported(data):
        return False
...
# --------------------------------------------------
# Extract text subtitles to external SRT
# --------------------------------------------------
...
def extract_subtitles(video_path: Path):
    ...
    for subtitle_index, stream in enumerate(subtitle_streams, start=1):
        ...
        try:
            ...
            log("SUB_EXTRACT_ERROR", show_name, f"{lang}: {e}")

        if subtitle_total:
            publish_preconvert_progress(video_path, "Extracting subtitles", (subtitle_index / subtitle_total) * 100, total_duration)

    if extracted_count > 0:
        log("SUB_EXTRACT_DONE", show_name, f"{extracted_count} subtitle(s)")
...
def convert(p, index_cache):
    ...
    show, season, filename = parse_show(p)
    title = f"{show} / {season} / {filename}"
    ...
    log(
        "SOURCE_INFO",
        title,
        f"video={video_codec} {width}x{height} bitrate={bitrate} audio={audio_codec} ch={audio_channels} duration={format_seconds(total_duration)}"
    )

    video_ok = video_stream_is_supported(video_stream, data)
    backend = None
    if not video_ok:
        backend, diagnostics = select_backend("h264", video_codec, pix_fmt)
        log(
            "VIDEO_BACKEND",
            title,
    ...
    profile_name = "VIDEO_COPY" if video_ok else "STANDARD_H264"
    ...
    if not matches_target_profile(tmp):
    ...
    for p in ROOT.rglob("*"):
        ...
        if is_partial_output(p):
            ...
            if final_path.exists() and not lock_is_active(final_path):
                log("DELETE_STALE_TEMP", f"{show} / {season} / {episode}", str(p))
                p.unlink()
            else:
                log("SKIP_TEMP_FILE", f"{show} / {season} / {episode}")
            continue
        ...
        try:
            if is_low_cpu_cost(p):
                ...
                if not is_indexed_media(p, index_cache):
                    ...
                    log("INDEX_EXISTING_OPTIMAL", f"{show} / {season} / {episode}")
                else:
                    log("SKIP_LOW_CPU_INDEXED", f"{show} / {season} / {episode}")
                continue
        except Exception as e:
            show, season, episode = parse_show(p)
            log("PROFILE_CHECK_ERROR", f"{show} / {season} / {episode}", str(e))

        title = " / ".join(parse_show(p))
        ...
        try:
            ...
            clear_progress()
            show, season, episode = parse_show(p)
            log("ERROR", f"{show} / {season} / {episode}", str(e))
